...
from parity.github import find_latest_workflow_run_id


@click.command("resolve-run-id", help="Look up the GitHub Actions run ID for a commit.")
@click.option("--repo", default=lambda: os.environ.get("GITHUB_REPOSITORY", ""), show_default="GITHUB_REPOSITORY")
@click.option("--workflow-id", default="parity.yml", show_default=True)
